...
function onOpen() {
  ...
  var fEasyMenuEntries = [ 
    ...
    {name: "Load ''From Addresses and Templates''", functionName: "loadAdressAndTemplates"}, 
...
function loadAdressAndTemplates(){
  /* Build an fromAddress dropdown list */
  ...
  var cell = toViewSheet.getRange(9, COL_FROM_ADDR);
  cell.setDataValidation(rule);
  
  Browser.msgBox("Load from addresses completed!")
  
  /* Build a templates dropdown list (Value In List Rule) */
  var draftMsgs = GmailApp.getDraftMessages();
  var msgSubjects = [];
  ...
  var rule_1 = SpreadsheetApp.newDataValidation().requireValueInList(msgSubjects).build();
 
  var cell = toViewSheet.getRange(9, COL_TEMPLATE);
  cell.setDataValidation(rule_1);
...
function loadGmailFromAdresses_Draft() {
  var aliases = GmailApp.getAliases()
